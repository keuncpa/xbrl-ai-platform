...
M1 결과의 **신뢰도**는 입력한 계정과목명과 표준 Taxonomy 레이블이 **텍스트로 얼마나 비슷한지**를 0~100%로 나타낸 점수입니다. 현재는 `difflib` 기반 글자 유사도로 계산하며(정확 일치 또는 과거 매핑 이력이 있으면 100%), 다음 기준으로 색이 구분됩니다.

| 신뢰도 | 표시 | 의미 |
|--------|------|------|
| 80% 이상 | 🟢 | 신뢰 가능 (정확히 일치하면 100%) |
| 50~80% | 🟠 | 부분 일치 — 사람이 확인 권장 |
| 50% 미만 | 🔴 + `EXT` | 표준에 마땅한 요소 없음 → 확장항목(Extension) 후보 |

**신뢰도가 낮은 이유**는 결국 하나입니다 — 엔진이 *뜻*이 아니라 *글자 모양*만 비교하기 때문입니다.

- 표현이 달라서: 예) `매각예정자산` ↔ 표준 `매각예정비유동자산`, `종업원급여자산` ↔ 표준 `순확정급여자산`.
- 내장 표준 사전이 작아서(현재 93개): 현실의 다양한 계정명과 가까운 표준이 없으면 점수가 낮아집니다.

> ⚠️ **신뢰도가 높다고 항상 정확한 것은 아닙니다.** 글자만 닮으면 높게 나오므로, 의미상 다른 요소에 매핑될 수 있습니다(예: `관계기업투자`가 손익 항목 `ShareOfProfitOfAssociates`에 붙는 경우). 따라서 주황·빨강 항목은 물론, 높은 점수도 검토 대상입니다.

**개선 방향**: `difflib`(글자 유사도)를 한국어 임베딩(의미 유사도)으로 교체하면 표현이 달라도 같은 개념을 잡아내 신뢰도·정확도가 함께 향상됩니다. 표준 Taxonomy 확충도 커버리지를 높이는 보완책입니다.
