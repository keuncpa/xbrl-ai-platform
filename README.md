...
│   │   ├── corps.py            #   전체 등록법인 검색 폴백 API (비상장 포함)
│   │   └── _engine/            #   xbrl_platform M1~M4 + Taxonomy 데이터 (vendored)
│   ├── scripts/
│   │   └── build_corps.py      #   빌드 시 DART 전체 법인 목록 → public/corps_all.json 생성
│   ├── vercel.json             #   서버리스 함수 설정 + 리전(icn1·서울) 지정
...
서버리스 함수는 **서울 리전(icn1)** 에서 실행됩니다 — DART API가 해외 리전에서는 매우 느리기 때문입니다.

### 기업 검색 데이터 (비상장 포함)

- 빌드 단계(`prebuild`)에서 `scripts/build_corps.py` 가 DART corpCode 전체 목록(약 10만+ 등록법인,
  비상장 포함)을 내려받아 `public/corps_all.json` 정적 파일로 생성합니다. 브라우저는 첫 검색 시
  이 파일을 1회 로드해 로컬에서 즉시 검색합니다.
- 정적 파일 생성에 실패한 배포에서는 `/api/corps?q=기업명` 서버리스 폴백이 동작합니다.
- 조회 시 연결(CFS) 재무제표가 없으면 별도(OFS)로 자동 재시도하며, 구조화 데이터를 제출하지 않는
  법인(감사보고서만 공시)은 재무제표 업로드 경로로 안내합니다.
